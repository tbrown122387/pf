...
void run_svol_comparison(const std::string &csv) {
  ...
  svol_bs<numparts, dimstate, dimobs,
          mn_resampler<numparts, dimstate, FLOATTYPE>, FLOATTYPE>
      bssvol(phi, beta, sigma);
  svol_apf<numparts, dimstate, dimobs,
           mn_resampler<numparts, dimstate, FLOATTYPE>, FLOATTYPE>
      apfsvol(phi, beta, sigma);
  svol_sisr<numparts, dimstate, dimobs,
            mn_resampler<numparts, dimstate, FLOATTYPE>, FLOATTYPE>
      sisrsvol(phi, beta, sigma);
  svol_bs_wc<numparts, dimstate, dimobs,
             mn_resampler<numparts, dimstate, FLOATTYPE>, FLOATTYPE>
      bssvolwc(phi, beta, sigma);
  ...
  // another identity lambda because the signature for "with covariate" models
  // is different
  ...
  for (size_t row = 0; row < data.size(); ++row) {
    ...
    std::cout << bssvol.getExpectations()[0] << ", "
              ...
              << bssvol.getLogCondLike() << ", " << apfsvol.getLogCondLike()
